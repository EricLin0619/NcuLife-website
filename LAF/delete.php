...
if (!((isset($_SESSION['LAF_user'])) && (isAuthorized("",$MM_authorizedUsers, $_SESSION['LAF_user'], $_SESSION['LAF_authority'])))) {   
  $MM_qsChar = "?";
...
$row_laf_data = mysqli_fetch_assoc($laf_data);
  ...
  $deleteSQL = sprintf("DELETE FROM `laf_data` WHERE `no`=%s",
                       GetSQLValueString($_GET['no'], "int"));

  mysqli_select_db($conn_LAF,$database_conn_LAF);
  $Result1 = mysqli_query($conn_LAF,$deleteSQL) or die(mysqli_connect_error());
  
  if (file_exists($row_laf_data['attachment'])){ unlink($row_laf_data['attachment']); }
