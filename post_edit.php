...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    if($_FILES['attachment1']['size'] > 0){
        ...
        if ($row_military_bulletin['attachment1'] != ''){ 
            ...
            unlink($row_military_bulletin['attachment1_name']);
        };
    ...
    if($_FILES['attachment2']['size'] > 0){
        ...
        if ($row_military_bulletin['attachment2'] !=''){ 
            ...
            unlink($row_military_bulletin['attachment2_name']);
        };
    ...
    if($_FILES['attachment3']['size'] > 0){
        if ($row_military_bulletin['attachment3'] !=''){ 
            ...
            unlink($row_military_bulletin['attachment3_name']);
        };
  ...
  $insertSQL = sprintf("UPDATE `military_bulletin` SET `time`=%s, `top`=%s, `day`=%s, `day2`=%s, `day_count`=%s, `day_count2`=%s, `day_post`=%s, `day_end`=%s, `class`=%s, `class2`=%s, `title`=%s, `content`=%s, `attachment1`=%s, `attachment1_name`=%s, `attachment2`=%s, `attachment2_name`=%s, `attachment3`=%s, `attachment3_name`=%s, `poster`=%s, `poster_real`=%s WHERE `no`=%s",
                            ...
                            for( $i=1 ; $i<=Max_File_Num ; $i++) { ?>
                                <tr>
                                    ...
                                    <td width="450" valign="top">
                                        <label>
                                            <input name="attachment<?php echo $i;?>" type="file" id="attachment"/>
                                            <input name="attachment<?php echo $i; echo '_name';?>" type="text" id="attachment<?php echo $i; echo '_name';?>" value="<?php if ($row_military_bulletin['attachment' . $i] !=''){
                                                $name = $row_military_bulletin['attachment' . $i . '_name'];  
                                                if($name) echo $name;
                                                else echo explode("/", $row_military_bulletin['attachment' . $i])[1];}?>"/>
                                        <?php if ($row_military_bulletin['attachment' . $i] !=''){ ?>
                                            <input type="checkbox" name="deleteAttachment<?php echo $i;?>" value="Dlt">刪除此附件
                                            <br><span style="color:red;">檔案已存在，可於方框內更改檔案名稱</span>
                                            <br><span style="color:red;">若需更新檔案再請上傳新檔案</span>
                                        <?php } ?>
                                        </label>
                                    </td>
                                </tr>
